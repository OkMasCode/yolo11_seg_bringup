from setuptools import find_packages, setup
import os
from glob import glob

package_name = 'yolo11_seg_bringup'

setup(
    name=package_name,
    version='0.0.0',
    packages=find_packages(exclude=['test']),
    data_files=[
        ('share/ament_index/resource_index/packages',
            ['resource/' + package_name]),
        ('share/' + package_name, ['package.xml']),
        (os.path.join('share', package_name, 'launch'), glob('launch/*.launch.py')),
    ],
    install_requires=['setuptools'],
    zip_safe=True,
    maintainer='sensor',
    maintainer_email='sensor@todo.todo',
    description='TODO: Package description',
    license='Apache-2.0',
    extras_require={
        'test': [
            'pytest',
        ],
    },
    entry_points={
        'console_scripts': [
            'clio_vision = yolo11_seg_bringup.clio_vision:main',
            'simple_yolo_annotator_node = yolo11_seg_bringup.simple_yolo_annotator_node:main',
            'pc_vision_node = yolo11_seg_bringup.pc_vision:main',
            'pc_vision_node_v2 = yolo11_seg_bringup.pc_vision_v2:main',
            'pc_vision_node_v3 = yolo11_seg_bringup.pc_vision_v3:main',
            'rgb_vision_node = yolo11_seg_bringup.rgb_vision_node:main',
            'vision_node = yolo11_seg_bringup.vision_node:main',
            'no_pc_vision_node = yolo11_seg_bringup.no_pc_vision:main',
            'clip_model_validator_node = yolo11_seg_bringup.clip_model_validator_node:main',
            'goal_detector = yolo11_seg_bringup.goal_detector:main',
            'mapper_node = yolo11_seg_bringup.mapper_node:main',
            'mapper_node_v2 = yolo11_seg_bringup.mapper_node_v2:main',
            'mapper_node_v3 = yolo11_seg_bringup.mapper_node_v3:main',
            'mapper_node_v4 = yolo11_seg_bringup.mapper_node_v4:main',
            'mapper_node_v5 = yolo11_seg_bringup.mapper_node_v5:main',
            'map_points_node = yolo11_seg_bringup.map_points_node:main',
            'clustered_map_points_node = yolo11_seg_bringup.clustered_map_points_node:main',
            'clip_reader = yolo11_seg_bringup.clip_reader:main',
            'goal_checker_node = yolo11_seg_bringup.goal_checker_node:main',
            'pointcloud_check_node = yolo11_seg_bringup.pointcloud_check:main',
            'detection_points_cloud_node = yolo11_seg_bringup.detection_points_cloud_node:main',
            'cpp_mapper_json_exporter_node = yolo11_seg_bringup.cpp_mapper_json_exporter_node:main',
        ],
    },
)
