...
class NoPCVisionNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f"Subscribed to: {self.image_topic} and {self.depth_topic}")
    ...
    def sync_callback(self, rgb_msg: Image, depth_msg: Image):
        """
        Synchronized RGB-Depth callback.
        """
    ...
    def process_frame(self, rgb_msg: Image, depth_msg: Image):
        """
        Process a single RGB frame with depth image.
        """
        ...
        try:
            ...
            self._process_detections(res, cv_bgr, depth_msg, rgb_msg, height, width)
    ...
    def _process_detections(self, res, cv_bgr, depth_msg, rgb_msg, height, width):
        """ 
        Unified pipeline using camera's depth image
        """
        ...
        try:
            ...
            if depth_m.shape[0] != height or depth_m.shape[1] != width:
                self.get_logger().warn(
                    f"Depth/RGB size mismatch: depth={depth_m.shape[:2]} rgb={(height, width)}. Resizing depth.",
                    throttle_duration_sec=5.0,
                )
                depth_m = cv2.resize(depth_m, (width, height), interpolation=cv2.INTER_NEAREST)

            depth_total_time = (perf_counter() - depth_start) * 1000

            self.timing_stats['pointcloud_conversion'].append(depth_total_time)
            self.get_logger().debug(f"Depth Conversion Total: {depth_total_time:.2f}ms")
    ...
    def process_single_detection(self, idx, bbox, class_id, class_name, instance_id, confidence, masks_np,
                                 ...
                                 height, width, rgb_msg, do_clip_frame):
        """ 
        Process geometry using camera's depth image and prepare data container. 
    ...
    def get_points_in_mask(self, depth_m, binary_mask, fx, fy, cx, cy):
        ...
        u = u[valid_z_mask].astype(np.float32)
        v = v[valid_z_mask].astype(np.float32)
        z = z[valid_z_mask].astype(np.float32)
