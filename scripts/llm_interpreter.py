...
def load_house_map(filename):
    ...
    if isinstance(data, dict):
        # map.json has structure: {"id": {"name": ..., "pose_map": ...}}
        result = []
        for obj_id, obj_data in data.items():
            result.append({
                "id": obj_id,
                "class": obj_data.get("name", ""),
                "coords": obj_data.get("pose_map", {})
            })
        return result
...
def compute_cluster_coords(cluster_id: int) -> Dict | None:
    ...
    for entry in clustered_map:
        ...
        if isinstance(centroid, dict):
            ...
            y = centroid.get("y")
            z = centroid.get("z")
            # Return if we have complete coordinate triple
...
def _build_alternatives(cluster_id: int, goal_class: str) -> List[Dict]:
    ...
    for entry in clustered_map:
        ...
        if not coords:
            # Fallback: find any map object of this class
            match = next((o for o in house_map if o.get("class") == cls), None)
            if match:
                coords = match.get("coords") or match.get("pose_map")
        if isinstance(coords, dict):
            alt["coords"] = coords
...
def save_robot_command(output_path: str, prompt: str, result: NavResult) -> None:
    ...
    payload = {
        ...
        "prompt": prompt,
        "goal": result.goal,
        "goal_objects": _objects_minimal(result.goal_objects),
        "cluster_info": cluster_info,
        "clip_prompt": result.clip_prompts,
...
def main():
    ...
    while True:
        ...
        try:
            ...
            print(f"Warning: failed to save robot command: {e}")
